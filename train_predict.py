...
from models import scnn, ernn, eattention,ecnn, inference, flowdata, predict
...
def experiment(
    ...
    assert model_name in ['scnn', 'ernn','eattention','ecnn','inference','flowdata','predict'], f'Model name error, {model_name}'   #增加模型
    # args
    N_STACK = n_stack # 堆叠
    N_DELAY = n_delay # 时延
    NUM_CPUS = num_cpus
    ...
    PREDICT_MODEL_PATH=pathConvert(f'./FlowData/{net_env}_{net_name}_{N_STACK}_{N_DELAY}.pkl')# 读取存储的模型文件
    ...
    policy_kwargs = dict(
        ...
        features_extractor_kwargs=dict(features_dim=32, PREDICT_MODEL_PATH=PREDICT_MODEL_PATH)# features_dim 提取的特征维数
    )
    model = PPO(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='predict')
